...
//		int nbRound=Integer.parseInt(args[2]);//求解次数
		Graph G=readDimacsInstance("/Users/davidlee/Downloads/luogu/src/main/java/DSJC500.5.col");//读取图文件
		int nbColor=Integer.parseInt("50");//颜色数
		int nbRound=Integer.parseInt("10");//求解次数
		System.out.println(G);
//		TabuColoring tabucol=new TabuColoring(G,nbColor);
		SimaColoring simacol=new SimaColoring(G,nbColor);
...
//		}
		for (int r = 1; r <= nbRound; r++) {
			int[] sol=simacol.search(); //执行搜索，返回解
			System.out.printf("Round %2d ",r);
			G.verifySolution(sol,nbColor); //验证解的正确性
		}
	}
	...
	public void verifySolution(int[] sol,int nbColor) {
		...
		for(int v=1;v<=this.verNum;v++) {
			...
			for(int neibor:this.adjList.get(v)) {
				...
				if(sol[neibor]==color){
					System.out.println("vertex "+v+" and vertex "+neibor+" have the same color "+color);
					System.out.println("failed!");
					return ;
				}
			}
